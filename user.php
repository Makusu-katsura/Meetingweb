...
/*$sqlSelectT = "SELECT CURRENT_DATE(),ADDTIME(CURRENT_TIME(),\"7:00:00\") FROM SelectedMeeting INNER JOIN AllMeeting ON AllMeeting.MeetingID=SelectedMeeting.MeetingID WHERE SelectedMeeting.UserID = 01000 AND SelectedMeeting.MeetConf=1 AND DATEDIFF(day,AllMeeting.Date,CURRENT_DATE())<0";
	$resultSelectT = mysqli_query($conn, $sqlSelectT);*/
	...
	if (mysqli_num_rows($resultSelectE) > 0) {
		while ($rowX = mysqli_fetch_assoc($resultSelectE)) {
			if ($rowX["Head"] == "CANCEL_PRIORITY") {
				...
				<h4>Meeting's Priority Chairman '" . $obj1["Name"] . " " . $obj1["Last_Name"] . "' has denied meeting '" . $rowX["MeetingID"] . "'</h4>
			...
			if ($rowX["Head"] == "ADMINSEND") {
				echo "<div id=\"MessageBox\">	
				<h4>Meeting '" . $rowX["MeetingID"] . "' has been denied </h4>
				<a onClick=\"this.parentElement.style.display='none';\" class=\"close\">OK</a>
				</div>";
