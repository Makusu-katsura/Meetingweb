...
<body>
	...
	<?php
		...
		array_push($pri,$_POST["priority"][$i]);}}
		for($i=0;$i<count($_POST["userid"]);$i++){
		if(trim($_POST["userid"][$i]) != ""){
		$sqlSelect2 = "INSERT INTO `SelectedMeeting`(`MeetingID`,`UserID`, `Priority` ) VALUES ('".$_POST["custId"]."','".$_POST["userid"][$i]."','".$pri[$i]."')";
		...
		$mail->Subject = 'HTML';
		$mail->Body = 'ท่านได้รับเชิญร่วมการประชุม   '.$meetingIn["Topic"].'  วันที่   '.$meetingIn["Date"].'  เวลา   '.$meetingIn["Time"].'  ที่  '.$meetingIn["Place"].'<a href="https://webserv.kmitl.ac.th/tn60010382/Meeting_Management/user.php";> Click Here to confirm';
	...
	$variable1=$variable2;
		...
		$meeting=$variable1;
		echo "<form f1.action=\"Add_user.php\" method=\"post\"  id=\"f1\" name=\"f1\" value=$meeting>
		<table id=\"r\"><tr><th><input type=\"checkbox\" onClick=\"toggle(this)\">All<br></th><th>Name</th><th>Last_Name</th><th>Position</th><th>Email</th><th>Priority</th>";
